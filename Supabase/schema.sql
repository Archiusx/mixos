...
-- ============================================================
-- CORRELATION ENGINE — cached cross-platform identity-resolution
-- reports (clusters, confidence scores, correlation graph) per case.
-- The report is computed client-side by src/correlationEngine.js
-- from investigations.data.findings; this table just persists the
-- last computed snapshot so it can be reviewed/exported without
-- recomputation and reused by future server-side jobs.
-- ============================================================

create table if not exists public.correlation_reports (
  id            bigint generated always as identity primary key,
  case_id       text not null,
  user_id       text not null,              -- Firebase UID of the case owner
  overall_confidence int not null default 0,
  tier          text not null default 'Weak' check (tier in ('Weak','Moderate','Strong','Very Strong')),
  risk_score    int not null default 0,
  report        jsonb not null default '{}', -- full buildCorrelationReport() output (clusters, graph, signals)
  created_at    timestamptz not null default now(),
  updated_at    timestamptz not null default now(),
  unique (case_id)
);

create index if not exists correlation_reports_case_id_idx on public.correlation_reports (case_id);
create index if not exists correlation_reports_user_id_idx on public.correlation_reports (user_id, created_at desc);

alter table public.correlation_reports enable row level security;

-- Same visibility rule as investigations: owner OR an accepted case_access grantee.
drop policy if exists "select own or shared correlation reports" on public.correlation_reports;
create policy "select own or shared correlation reports"
  on public.correlation_reports for select
  using (
    (select auth.jwt()->>'sub') = user_id
    or exists (
      select 1 from public.case_access ca
      where ca.case_id = correlation_reports.case_id
        and lower(ca.grantee_email) = lower((select auth.jwt()->>'email'))
        and ca.status = 'accepted'
    )
  );

-- Owner OR accepted editor may write (insert/update) the cached report.
drop policy if exists "upsert own or shared-editor correlation reports" on public.correlation_reports;
create policy "upsert own or shared-editor correlation reports"
  on public.correlation_reports for insert
  with check (
    (select auth.jwt()->>'sub') = user_id
    or exists (
      select 1 from public.case_access ca
      where ca.case_id = correlation_reports.case_id
        and lower(ca.grantee_email) = lower((select auth.jwt()->>'email'))
        and ca.role = 'editor' and ca.status = 'accepted'
    )
  );

drop policy if exists "update own or shared-editor correlation reports" on public.correlation_reports;
create policy "update own or shared-editor correlation reports"
  on public.correlation_reports for update
  using (
    (select auth.jwt()->>'sub') = user_id
    or exists (
      select 1 from public.case_access ca
...
drop policy if exists "owner deletes correlation reports" on public.correlation_reports;
create policy "owner deletes correlation reports"
  on public.correlation_reports for delete
  using ((select auth.jwt()->>'sub') = user_id);

alter publication supabase_realtime add table public.correlation_reports;
